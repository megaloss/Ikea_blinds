...
step_timerid = tmr.create() -- which timer to use for the steps
status_timerid = tmr.create() -- timer id for posing of status messages
-- setup pins
...
--  if cur_step > 0 and cur_step < tot_steps and step_stepsleft > 0 then
    step_timerid:alarm (2, 0, step_take )
  else
    step_stopstate()
    step_stop()
    pins_disable()
...
function percent_go_to(percent, delay)
  if(percent >= 0 and percent <= 100) then
    step_stop()
    status_timerid:register (1000, tmr.ALARM_AUTO, function () mq.post_status() end)
    status_timerid:start()
